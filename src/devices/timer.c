...
timer_sleep (int64_t ticks) 
{
  ...
  // To disable interrupts around the assignments.
  enum intr_level old_level;
  
  ASSERT (intr_get_level () == INTR_ON);
  old_level = intr_disable();
  thread_current()->state_time = start + ticks;

  // For reference for thread which are sleeping
  list_push_back(&sleep_thread, &thread_current()->elem);
  thread_block();
  intr_set_level(old_level);
/*
