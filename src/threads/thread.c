...
thread_init (void) 
{
  ...
  // The system load average (load_avg) estimated the average number of threads ready to run
...
{
  // Through thread_yield() its priority is the highest one
  ...
  if(new_priority < highest_priority) thread_yield();
}
...
void recalculate_priority(void){
  ...
  for (e = list_begin(&all_list); e != list_end(&all_list); e = list_next(e)) {
    ...
    t->priority = f_sub_f(f_sub_f(f_add_i(0, PRI_MAX), f_div_i(t->recent_cpu, 4)), i_mul_f(2, f_add_i(0, t->nice))) / FIXED;
    
    // Update the priority within the range of priority ( 0 (PRI_MIN) - 63 (PRI_MAX) )
...
void update_load_avg(void){
  ...
  load_avg = f_div_i(f_add_i(i_mul_f(59, load_avg), ready_threads), 60);
}
  ...
  for (e = list_begin(&all_list); e != list_end(&all_list); e = list_next(e)) {
      t = list_entry(e, struct thread, allelem);
        ...
        if (t != idle_thread) {
          ...
          t->recent_cpu = f_add_i(f_mul_f(f_div_f(i_mul_f(2, load_avg), f_add_i(i_mul_f(2, load_avg), 1)), t->recent_cpu), t->nice);
        }
  }
...
{
  /* Proj 3 */
  return i_mul_f(100, load_avg) / FIXED;
}
...
{
  /* Proj 3 */
  return i_mul_f(100, thread_current()->recent_cpu) / FIXED;
}

...
init_thread (struct thread *t, const char *name, int priority)
{
  ...
  /* proj 2 */
#ifdef USERPROG  
  for(i = 0; i < 130; i++) t->file_list[i] = NULL;
  t->exit_status = -1;
