...
process_execute (const char *file_name) 
{
  ...
  strlcpy(filename_copy, file_name, 129);
  fn = strtok_r(filename_copy, " ", &next_ptr);


  /* Exec-missing */
  if(filesys_open(fn) == NULL) {
    return -1;
  }
  ...
  for (e = list_begin(l); e != list_end(l); e = list_next(e)) {
    ...
    /* Check the child thread if it's terminated in failure. */
    if(child_thread->isTerminated == true) return process_wait(tid);
  }
...
start_process (void *file_name_)
{
  ...
  /* Acquires lock for the current thrad, 
      first waiting for any current owner to release it if necessary*/

  success = load (file_name, &if_.eip, &if_.esp);
  /* Release lock, which the current thread must own. */
...
process_wait (tid_t child_tid UNUSED) 
{
  ...
  /* Check if child_tid is valid or not */
  if(child_tid == TID_ERROR) return -1;
  if(child_tid < 0) return -1;

  /* For 'wait-twice', if it calls wait more than one time, 
      it immediately returns -1 (The first call must wait in the usual way and return the exit code.
      The second wait call must return -1 immediately.) */
   if(wait_count != 0) return -1;

  /* Search for the child thread corresponding to the given child_tid */
    ...
    if(child_tid != child_thread->tid){
      /*do nothing */
		...
		else if( child_tid == child_thread->tid && child_thread->parent == cur){
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  /* analyzing and tokenizing the file */
  strlcpy(filename_copy, file_name, 129);
  ptr = strtok_r(filename_copy, " ", &next_ptr);
  while(ptr != NULL){
    argv[argc++] = ptr;
  ...
  /* Argument Passing : making up the stack */
  ...
  **(uint32_t **)esp = 0;
  ...
  /* argv[3], argv[2], arg[1], argv[0] */

  for(i = argc - 1; i >= 0; i--){
    esp_copy -= (strlen(argv[i]) + 1);
    *esp -= 4;
    **(uint32_t **)esp = (uint32_t)esp_copy;  
  }
